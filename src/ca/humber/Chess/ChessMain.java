 ...
 * *
 *
 * Program runs, but is missing components: No checks/checkmates but wasn't required, 
 * all pieces behave as they should in regards to movement and capture except for
 * the queen *does not move properly but does have movement*, undesirable inputs will crash 
 * the game *anything non numeric that includes a space*. Unlikely the game would crash 
 * without trying to make it do so. Pawns do not have en passant movement, and no pawn promo.
 *
 * I'm sure with more time and practice could make even better, but this is what was able to
 * be done.
 *
 * - Anthony
 *
...
public abstract class ChessMain {
    ...
    public static void main(String[] args) {
        ...
        //of the program. And given a color that the game will use
        //to ensure they can only move pieces that match and belong to them.
        ...
        //unfortunately could not figure out a way to implement working captures for all pieces
        //there is no way to win so the game loops infinitely you can only move pieces around
        //and capture pieces clearing the board.
        while (true) {
